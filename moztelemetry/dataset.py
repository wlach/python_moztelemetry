...
def _group_by_size_greedy(obj_list, tot_groups):
    """Partition a list of objects using a greedy strategy

    The local optimum choice is to always put an object in the smallest bucket.
    ...
    for obj in sorted_list:
        smallest_group = min(groups, key=lambda x: x[0])
        smallest_group[0] += obj['size']
        smallest_group[1].append(obj)
